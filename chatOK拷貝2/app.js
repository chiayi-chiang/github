...
app.get('/getdata', (req, res) => {
    con.query('SELECT user.userName ,sendMessage.sendTime,sendMessage.message,sendMessage.touXiangUrl FROM user JOIN `sendMessage` ON sendMessage.whosend=user.userID WHERE roomID=0 ORDER BY `sendMessage`.`messageID` DESC limit 50', function (error, results, fields) {
        let sendmessage = JSON.stringify(results);//將SELECT出來的資料字串化
        // console.log(sendmessage);
        res.send(sendmessage);//回傳給main.js
    });
});
        ...
        for(let _user of users) {// for-in是得到每一個key，for-of 是得到每一個value
            if(_user.username === data.username) {
                ...
                con.query('SELECT `userID` FROM `user` WHERE userName = ?',[data.username],function(error, results, fields){
                    ...
                    con.query('INSERT INTO `sendMessage` SET ? ', postforDB);
                });
                let postmessage = {
                    username: data.username,
                    message: data.message,
